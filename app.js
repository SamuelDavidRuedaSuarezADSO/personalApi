...
import { enviar, documentos, listar } from "./modelo.js";
...
const validar = (event) => {
  ...
  if (name === "" || apell === "" || tipe === "pre" || doc === "" || mail === "" || dicc === "" ) {
    alert("Error: Ingresa todos los campos");
  }
  else {
    // Validacion de que no ingresen numeros al campo nombre
    if (isNaN(name)) {
      // Validacio de que no ingresen numeros al campo apellido
      if(isNaN(apell)){
        ...
        // Si la validacon del apellido es incorrecta emite esta alerta
